...
public:
	...
	void inc_count(int pos, float weight = 1){
		count[pos] = count[pos] + weight;
	}
	void dec_count(int pos, float weight = 1){
		count[pos] = count[pos]-weight;
	}
	...
	vector<float> get_count(){
		return count;
	}
	float get_count(int pos){
		return count[pos];
	}
	...
	while ( !dFile.eof() )
	{
		...
		if(store >= 0){
				...
				for(int k=0; k< node.get_nvalues(); k++){
					if(assigned_val[store] == k) unknown_val.push_back(1);
					else unknown_val.push_back(0);
				}
...
int EM(network &Alarm){
	...
	vector<Graph_Node>::iterator listIt;
	for(listIt = Alarm.Pres_Graph.begin(); listIt!= Alarm.Pres_Graph.end(); listIt++){
		vector<float> cnt = listIt->get_count();
		vector<float> CPT = listIt->get_CPT();
	...
	for(int i=0; i<unknowns.size(); i++){
		...
		vector<float> old_val;
		for(int j=0; j<nvalues; j++){
			old_val.push_back(unknowns[i][1+j]);
		}
		...
		for(int k=0; k < nvalues; k++){
			float prob = Alarm.Pres_Graph[node].get_CPT(k*offset + unknowns[i][1+nvalues]);
			for(int j=0; j<children.size(); j++){
				prob *= Alarm.Pres_Graph[children[j]].get_CPT(unknowns[i][2+nvalues+j*2] + k*unknowns[i][2+nvalues+j*2+1]);
			}
			new_val.push_back(prob);
			prev_prob += prob;
			// new_val.push_back(0);
			// if(m_prob < prob){
			// 	m_prob = prob;
		...
		// decrease count
		for(int j=0; j<nvalues; j++){
				Alarm.Pres_Graph[node].dec_count(j*offset + unknowns[i][1+nvalues], old_val[j]);
			}
		...
		for(int j=0; j<children.size(); j++){
			for(int k=0; k<nvalues; k++){
						Alarm.Pres_Graph[children[j]].dec_count(unknowns[i][2+nvalues+j*2] + k*unknowns[i][2+nvalues+j*2+1], old_val[k]);
					}
		...
		// increase new count
		for(int j=0; j<nvalues; j++){
				Alarm.Pres_Graph[node].inc_count(j*offset + unknowns[i][1+nvalues], new_val[j]);
			}
		for(int j=0; j<children.size(); j++){
			for(int k=0; k<nvalues; k++){
						Alarm.Pres_Graph[children[j]].inc_count(unknowns[i][2+nvalues+j*2] + k*unknowns[i][2+nvalues+j*2+1], new_val[k]);
					}
		}

		// update unknowns
		for(int j=0; j<nvalues; j++){
				unknowns[i][1+j] = new_val[j];
				if(new_val[j] > old_val[j]) update_cnt = update_cnt + new_val[j] - old_val[j];
				else update_cnt = update_cnt - new_val[j] + old_val[j];
			}
